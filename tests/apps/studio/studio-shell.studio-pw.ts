...
const studioScreenStateAnchors = [
  ...
  { id: "article-directory-populated", name: "Articles", role: "heading" },
  {
  ...
  { id: "media-browser", name: "Media", role: "heading" },
  {
  ...
  { id: "settings-saved", name: "Settings", role: "heading" },
...
test("keeps sidebar and preview panel controls command-backed", async ({
  ...
  await expect(page.getByLabel("Sidebar collapsed")).toBeVisible();

  const collapsedSidebarBox = await page
    .getByLabel("Sidebar collapsed")
    .boundingBox();
  ...
  await page.getByRole("button", { name: "Toggle preview" }).first().click();
  await expect(page.getByLabel("Preview collapsed")).toBeVisible();

  const collapsedPreviewBox = await page
    .getByLabel("Preview collapsed")
    .boundingBox();

  expect(collapsedPreviewBox?.width).toBeLessThan(100);
  await expectNoDocumentOverflow(page);
});
...
test("renders descriptor-backed settings sections and local draft state", async ({
  ...
  await expect(work.getByRole("heading", { name: "Settings" })).toBeVisible();
  await expect(work.getByText("Settings saved automatically")).toBeVisible();
...
test("renders dirty and invalid settings as local, actionable states", async ({
  ...
  await expect(
    work.getByText("Decide which top-level destinations"),
  ).toBeVisible();
  await expect(
  ...
  await expect(work.getByText("Control the public URL")).toBeVisible();
...
test("renders article directory cards, filters, and warning markers", async ({
  page,
}) => {
  await page.setViewportSize({ height: 900, width: 1440 });
  ...
  await expect(work.getByRole("heading", { name: "Articles" })).toBeVisible();
  await expect(
  ...
  await work.getByRole("button", { name: "Open editor" }).first().click();
  await expect(
...
test("renders the article editor metadata form, source editor, and preview", async ({
  page,
}) => {
  await page.setViewportSize({ height: 900, width: 1440 });
  ...
  await expect(work.getByLabel("Title")).toHaveValue(
...
test("keeps editor toolbar and context menu actions command-backed", async ({
  ...
  await expect(page.getByText("Cursor actions")).toBeVisible();
...
test("opens the command palette from toolbar search and invokes shared commands", async ({
  ...
  await palette
    ...
    .fill("media");
  await expect(palette.getByText("Open the media library.")).toBeVisible();
  await palette
    .getByRole("option")
    .filter({ hasText: "Open the media library." })
    .click();
...
test("renders command-backed dropdowns, disabled reasons, and shortcut tooltips", async ({
  ...
  await expect(
    tooltip.getByText("Show or hide the left sidebar."),
  ).toBeVisible();
...
test("renders invalid article editor state with blocked publish affordances", async ({
  ...
  await expect(work.getByText("This article has errors")).toBeVisible();
  await expect(work.getByLabel("Title")).toHaveValue("");
  await expect(
...
test("renders unsupported body and preview-collapsed writing modes", async ({
  page,
}) => {
  await page.setViewportSize({ height: 768, width: 1024 });
  ...
  await page.getByRole("button", { name: "Toggle preview" }).first().click();
  await expect(page.getByLabel("Preview collapsed")).toBeVisible();
  await expect(work.getByText("Focused writing mode")).toBeVisible();
  await expect(
    work.getByRole("button", { name: "Open preview" }),
  ).toBeVisible();
  await expectNoDocumentOverflow(page);

  await test.info().attach("studio-article-editor-preview-collapsed", {
    body: await page.screenshot({ fullPage: true }),
    contentType: "image/png",
  });
});
...
test("renders media browser grid, list, detail, and missing-alt states", async ({
  ...
  await expect(work.getByRole("heading", { name: "Media" })).toBeVisible();
...
test("routes publish through preview, confirmation, progress, and success", async ({
  ...
  await expect(
    dialog.getByText("Studio is using a redacted credential reference."),
  ).toBeVisible();
  ...
  await expect(
    work.getByText("This prototype does not send credentials or deploy output"),
  ).toBeVisible();

  await work.getByRole("button", { name: "Show success state" }).click();
  await expect(
...
test("renders checkpoint restore selection, confirmation, and completion", async ({
  ...
  await expect(
    work.getByText("No filesystem or provider writes"),
  ).toBeVisible();
...
test("renders preview-ready and preview-stale route states", async ({
  ...
  await previewPane
    .getByRole("button", { name: "Open preview externally" })
    .click();
  await expectNoDocumentOverflow(page);
...
test("opens a home-route preview from project context", async ({ page }) => {
  await page.setViewportSize({ height: 900, width: 1440 });
  await openStudio(page, "/?screen=project-home");
  ...
  await expect(previewPane.getByText("/", { exact: true })).toBeVisible();
  await expect(
    previewPane.getByRole("heading", { name: "Northwind Journal" }),
  ).toBeVisible();
  await expect(
    previewPane.getByRole("heading", { name: "Home page preview" }),
  ).toBeVisible();
  await expectNoDocumentOverflow(page);
