...
| Rust workspace checks              | `just rust-check`, `bun run rust:check`, direct `cargo fmt/check/clippy/test` commands             | Additive Rust formatting, type, lint, doctest, unit-test, and CI gates before Rust replaces any Bun behavior. |
