...
- `docs/governance/SUPPLY_CHAIN_AND_SECRET_POLICY.md` owns repo-level dependency,
  lockfile, secret-scan, public-env, and third-party-script posture.
- `docs/governance/STATIC_OUTPUT_SECURITY.md` owns browser-facing static-output trust
  boundaries.
- `docs/operations/DEPLOYMENT_ADAPTER_CONTRACT.md` owns deploy request/result shapes and
  requires credential references rather than secret values.
- `docs/operations/EXTENSION_ARCHITECTURE.md` owns extension manifests, capability
  declarations, permissions, and lifecycle states.
- `agent-docs/studio/STUDIO_ADAPTER_MODEL.md` owns provider adapter families.
