...
- Verification owner: `test:docs-site`, Markdown review, link checks when
  available, and generated-reference drift checks after `IRK-104`.
...
Current generated-reference precedent:
...
- **Site config reference:** generated from `siteConfigSchema`, with examples
  kept hand-written.
- **Route and feature reference:** generated from `routeRegistryEntries()` and
  site route defaults.
- **Source and artifact reference:** generated from
  `sourceArtifactManifest()` for active and fixture site instances.
- **Command reference:** generated from `package.json` and the QA command
  registry, replacing duplicated script classifications where possible.
- **Component inventory reference:** generated from `src/components/**`,
  `docs/components/**`, catalog fixtures, and component tests.
- **Article/content frontmatter reference:** generated from Astro content
  collection schemas once those schemas expose author-facing labels and
...
Future documentation tooling should add a docs accountability check that maps
changed source domains to expected documentation surfaces. That check should
use the ownership and trigger model in this document rather than hard-coded
filename guesses.

## Downstream Boundaries

This document completes the lifecycle design for `IRK-101`. It intentionally
does not implement downstream generated references or drift tooling.

- `IRK-102` should implement schema, route, feature, and command references
  after confirming the owning schemas and registries expose the right public
  descriptors.
- `IRK-103` should connect site-doctor diagnostics to author/site-owner docs
  after the diagnostic taxonomy is stable.
- `IRK-104` should add docs drift, link, and generated-reference checks after
  generated references exist.
