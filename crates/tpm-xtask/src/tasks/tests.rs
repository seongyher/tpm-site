...
fn write_distribution_docs(root: &Path) -> Result<(), Box<dyn Error>> {
    ...
    write_text(
        &root.join("docs/studio/STUDIO_TAURI_DISTRIBUTION.md"),
        "# Fixture\n\nsigning\nnotarization\nupdate\nsecret\n",
    )?;
    write_text(
...
fn distribution_readiness_reports_artifact_and_recipe_drift() -> Result<(), Box<dyn Error>> {
    ...
    fs::remove_file(root.join("docs/cli/CLI_DISTRIBUTION.md"))?;
    ...
    let text = issues.join("\n");
    assert!(
        text.contains("docs/cli/CLI_DISTRIBUTION.md: required distribution document is missing")
    );
